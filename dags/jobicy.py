...
from custom.operator_jobicy import JobicyGetJobsOperator
from custom.operator_json2sql import Json2SqlOperator


with DAG(
    dag_id="jobicy_get_jobs",
    start_date=dt.datetime(2023, 10, 13),
    schedule_interval="@daily",
    tags=['jakluz-de2.3'],
    catchup=False
) as dag:
    JobicyGetJobsOperator(
        task_id="get_jobs",
        conn_id="jobicy",
        start_date="{{ ds }}",
        output_path="/tmp/job_offers/data/jobicy.json"
    )


with DAG(
    dag_id="jobicy_parse_and_upload",
    ...
    upload_to_db = PythonOperator(
        ...
        op_kwargs = {
            'sql_filename': '/tmp/job_offers/data/jobicy-{{ data_interval_start }}.sql',
